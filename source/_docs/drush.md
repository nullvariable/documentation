...
The `drush sql-cli` command cannot be used in interactive mode on Pantheon. You may run individual queries non-interactively, though:
```bash
echo 'show tables;' | drush @pantheon.SITENAME.ENV sql:cli
```
